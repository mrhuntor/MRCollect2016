...
> - 1: 由于提交太多,[什么样的mr才是合格的mr](#什么样的mr才是合格的mr)的反例不再更新, 总提交数统计可能也会出错...所以发现有问题的请在提交截止前单独发非提交邮件指出
...
- 能反映2对(或多对)输入输出间关系
	- 反例1: *把同一个输入运行两遍, 结果相同*
	- 反例2: *对于任何输入, 输出等于输入的正弦值*
	- 反例14: *A1的输出结果为B1，det(A1) = det(B1)* [17](#题目名称-17matrix-transpose矩阵转置)
	- 反例23: *输入s0，s0，s1 输出d 结果为s1* [26](#题目名称-26words-replacement单词替换)
	- 反例34: *输入n<=0，输出0* [27](#题目名称-27candy-eating吃糖果问题)
	- 反例35: *对任意输入和输出，A[0][0]==B[0][0]* [17](#题目名称-17matrix-transpose矩阵转置)

- 根据MR能生成无穷多对输入输出对
	- 反例3: *当第1个输入等于-1, 第2个输入等于1时, 两个输出相加等于0*
	- 反例6: *对角矩阵的转置矩阵等于它本身* [17](#题目名称-17matrix-transpose矩阵转置)

- 不要万能MR
	- 反例4: 对于图相关的程序: *改变图点/边的输入顺序, 输出不变*
	- 反例5: 对于机器学习相关的程序: *改变训练集数据的顺序, 输出不变*
...
- 不一定正确的MR
	...
	- 反例17: *输入G，输出为y；向G中添加一个新点v，该点与y中所有的点建立边，得到G1；输入G1，得到输出y1； |y| == |y1|* [48](#题目名称-48vertex-covering-set点覆盖集)
	- 反例17: *输入G，输出为y；向G中添加一个新点v，该点与G中所有的点建立边，得到G1；输入G1，得到输出y1； |y| == |y1| - 1* [48](#题目名称-48vertex-covering-set点覆盖集)
	- 反例18: *输入 x，输出s； 去除x中最后一个元素v得到x'；在x后添加一个正整数u得到x''， 分别输入x',x''，得到s'，s'';若s' == s， 则 s'' == s；否则 s' == s-v，s'' == s + u* [18](#题目名称-18max-sum连续子数组的最大和)
	- 反例19: *删去G中任一结点v得到G', 与v相连的边的最小权为k， 输入G'，输出c'；c' == c - k* [21](#题目名称-21minimal-spanning-tree最小生成树)
	- 反例25: *当查找的元素不为第一个元素时，删除第一个元素所得的A'查询到的新的y'=y-1* [1](#题目名称-1binary-search-on-sorted-array有序数组的二分查找)
	- 反例27: *对于输入(A, x, k),输出为y,那么对于输入(A,x+1,k)输出为y2，如果y2等于-1，那么y2<=y1(包含y1=-1的情况),如果y2不等于-1，那么y2>y1(包含y1=-1的情况)* [2](#题目名称-2k-th-occurrence-of-x-in-unsorted-arrayx在无序数组中的第k次出现)
	- 反例28: *向图G中任意两条点之间添加两条连线形成新的图G';输入G,输出y; 输入G',输出y';y=y'* [23](#题目名称-23euler-circle欧拉回路)
	- 反例29: *输入x1得到y1，那么x1取反得到x2,输入x2得到y2，那么将y2取反后与y1相反，可以通过遍历验证* [20](#题目名称-20binary-search-tree二叉搜索树)
	- 反例30: *输入s , 输出p；在s 两端加上两个相同的字符ch，构成新的字符串s';输入s' , 输出p';则p=p'或者 p' = ch | p | ch或者p' = ch | p[0:len(p)-1]或者p' = p[len(s) - len(p):len(s) -1] | ch('|'是字符串连接符)* [33](#题目名称-33plalindrome最长回文子序列)
	- 反例33: *输入s，输出p，输入strcat(s,s)，输出strcat(p,p)* [33](#题目名称-33plalindrome最长回文子序列)
	- 反例36: *输入(S0, S1, S2)，输出为d1；输入(d1, S2, S3)，输出为d2；……；输入(dn-1, Sn, S1)，输出为dn。则dn == S0。* [26](#题目名称-26words-replacement单词替换)
	- 反例37: *取一个团Kn (n > 1)，将C中的所有点与G中所有的点建立边，得到G1；输入G1，输出为Y1；则有 Y1 == Y + Kn* [51](#题目名称-51max-clique最大团)

- 不完整的MR
	- 反例9: *输入 (A, n, x)， 输出 y; 1. 若 y != -1 && y != n-1，将 (A[y+1:n], n-y-1, x') 作为新的输入，其中x' < x, 输出 y' == -1. * [1](#题目名称-1binary-search-on-sorted-array有序数组的二分查找)
	- 反例10: *输入 (A, n, x)， 输出 y; 2. 若 y != -1 && y != 0，将 (A[0:y-1], y-1, x'') 作为新的输入，其中x'' > x, 输出 y' == -1.* [1](#题目名称-1binary-search-on-sorted-array有序数组的二分查找)
	- 反例11: *输入G，输出y; 若y=TRUE，则在G中任意两个节点添加一条边，输出y'=FALSE* [23](#题目名称-23euler-circle欧拉回路)
	- 反例12: *输出线段1、线段2，输出(x, y) 若两者相交，将线段1的横座标平移d个距离 输出为不相交或(x', y')，x'=x+d* [74](#题目名称-74line-segment-intersection线段相交)
	- 反例13: *输入A，B，输出y 输入A', B'，输出y' 若y=TRUE && y'=TRUE，输入[A A']，[B B']，输出TRUE* [77](#题目名称-77stack-game栈)
	- 反例15: *n的输出结果为y1，n+1的输出结果为y2，n+2的输出结果为y3；若y1 % 2 == 0，则y2 % 2 == 1，y3 % 2 == 1；若y2 % 2 == 0，则y1 % 2 == 1，y3 % 2 == 1；若y3 % 2 == 0，则y1 % 2 == 1，y2 % 2 == 1；* [27](#题目名称-27candy-eating吃糖果问题)
	- 反例16: *对 AB 一组结果，|A|大于等于 2，若 B[0]>B[1]且 y = true，则把 B[0]B[1]交换位置生成C，AC 结果 y 为 true;实际上，我们有对于出栈序列中的每一个数字，在它后面的、在 A 中下标比它大的所有数字，一定是按下标递减顺序排列的* [77](#题目名称-77stack-game栈)
	- 反例22: *输入s0，s1，s2，输出d 如果s1，s2皆为“”（空）则s0==d* [26](#题目名称-26words-replacement单词替换)
	- 反例26: *分别输入(A,x,1)~(A,x,m+1),其中(A,x,m+1)是第一个使输出y=-1的输入;如果m>0,构造数组A1为A的倒置,输入(A,x,k)输出a;输入(A1,x,m-k)输出b;当0<k<m则(a+b)==(m+1)* [2](#题目名称-2k-th-occurrence-of-x-in-unsorted-arrayx在无序数组中的第k次出现)
	- 反例31: *输入图G，输出根节点编号。新图G'在G基础上添加一个指向根节点的新节点，编号为n。输入新图G'，输出n* [59](#题目名称-59root-tree根树)

- 非常奇怪的MR
	- 反例20: *取出G中的结点，添加边使得G'连通且存在欧拉回路。 将G'中的边添加到G中（两结点间可能会有多条边），得到G'', G 和 G''的输出结果相同* [23](#题目名称-23euler-circle欧拉回路)

- 没有看懂的MR
	- 反例21: *给T中每个叶结点增加两个儿子，得到T’， 输入T’， 得到 s’。 s 中每个元素v是 s’ 中两个元素的前缀，且这两个元素分别为v0，v1* [61](#题目名称-61precode-of-binary-tree二叉树的二元前缀码)
	- 反例24: *将A分成A1和A2 先查询（A1,x,k）得y1 若y1等于y则成立;若y1=-1 查询（A1,x,k-1）...(A1,x,t)直到输出y2不为-1为止，再查询(A2,x,1)...（A2,x,n）直到n+t=k 得（A2,x,n）=y3;y=y3+y2;不然则输出-1* [2](#题目名称-2k-th-occurrence-of-x-in-unsorted-arrayx在无序数组中的第k次出现)
...
> - [mrhuntor] 如果输入(A, x, k), 输出y等于-1, 则输入(A, x, k+1), 输出也等于-1;
...
> - [mrhuntor] 交换比第一个元素大的第一个元素和比第一个元素小的第一个元素, 构造的二叉搜索树相同.【其实就是交换根节点的左右子节点的输入顺序】
...
> - [mrhuntor] n颗糖果的吃法总数加上n+1颗糖果的吃法总数等于n+2颗糖果的吃法总数
...
> - <del> 输入G得到输出为p。向G中顶点数较少的一侧加入一个点，并与另一侧的所有点构造边，得到G1；输入G1输出为q；
若G1两部点数相等：|q| == |p| + 1; 否则，|P| == |q|</del> (DEL BY 131220088)
